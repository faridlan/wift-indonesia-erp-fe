...
function numberToWords(num: number): string {
  ...
  if (num < 200)
    return "seratus" + (num % 100 ? " " + numberToWords(num % 100) : "");
  if (num < 1000)
    return (
  ...
  if (num < 2000)
    return "seribu" + (num % 1000 ? " " + numberToWords(num % 1000) : "");
  if (num < 1000000)
    return (
...
export function generateKwitansiPDF({
  ...
  drawRow(
    "No. Kwitansi",
    `KW-${String(order.order_number).padStart(4, "0")}-${payment.id.slice(0, 4).toUpperCase()}`,
    y,
  );
  y += 10;
  ...
  doc.setFontSize(18);
  doc.text(formatCurrency(payment.amount), pageWidth - margin - 8, y + 12, {
    align: "right",
  });
  ...
  doc.text(
    `Terbilang: ${terbilangText.charAt(0).toUpperCase() + terbilangText.slice(1)}`,
    margin + 8,
    y + 23,
  ...
  doc.text(`${COMPANY.name} — ${COMPANY.website}`, pageWidth / 2, footerY + 5, {
    align: "center",
  });
